...
6. [Draft PR #5](https://github.com/hamthet/WinSidebar/pull/5) — two window-management dialogs were repositioned on a locally tested/pushed feature commit; subsequently the owner reproduced a **right-click crash** (`ObjectDisposedException` for `ContextMenuStrip`). See [`BUG-2026-09-22-CONTEXT-MENU.md`](BUG-2026-09-22-CONTEXT-MENU.md): a narrowly scoped deferred-disposal patch script exists on the same branch, but has NOT been run or GUI verified. Do not mark the crash fixed or merge PR #5 yet.
...
- **Immediate blocker:** resolve reproduced context-menu right-click crash, run local smoke/publish, and verify repeated mouse/keyboard context invocation and dialog placement on owner's Windows; retain FAIL until owner confirms.
