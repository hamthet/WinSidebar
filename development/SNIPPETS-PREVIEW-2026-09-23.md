...
## Owner follow-up — Ctrl+double-quote sidebar opener

The first attempt using `Alt+"` did not work in the owner's real Windows test. The owner chose `Ctrl+"` instead.

Current source-staged contract:

- `Ctrl+"` opens the sidebar when it is collapsed;
- implementation registers Ctrl+Shift+`Keys.Oem7` so the double-quote key can be used;
- if the sidebar is already open, the command is a no-op;
- F1..F4 shortcut keys and configured Shift+F script keys are preserved.
...
## Owner follow-up — AltGr+Y sidebar opener

The owner reported that the staged `Ctrl+"` opener also failed in the real Windows test and proposed `AltGr+Y` instead. The implementation now registers `Ctrl+Alt+Y`, which is the Win32 modifier form generated by AltGr on the target keyboard layout.
